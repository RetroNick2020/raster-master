/* Eg3.c Raster Master Bob demo for vbcc                           */
/* vc vsprite.c -o vsprite -lauto -lamiga                            */
...
struct NewWindow my_new_window=
{
  ...
  "RM Bob Demo - Use the arrow keys to move the Bob!", /* Title */
...
void clean_up()
{
  /* Remove the VSprites: */
  if (bob_on) RemBob(&b);
  if (b.SaveBuffer != NULL) FreeMem(b.SaveBuffer,sizeof(img));
...
int main()
{
  ...
  vsprite.Flags =  SAVEBACK | OVERLAY; /* It is a BOB.            */
  vsprite.X = x;           /* X position.                 */
  vsprite.Y = y;           /* Y position.                 */
  vsprite.Height = 32;     /* 16 lines tall.              */
  ...
  vsprite.ImageData = img;
  vsprite.VSBob = &b;
  vsprite.BorderLine = Bline;
  vsprite.CollMask = Cmask;

  /* Pointer to the colour table: */
  vsprite.SprColors = NULL;

  b.Flags = 0;
  b.SaveBuffer = (WORD *)AllocMem(sizeof(img), MEMF_CHIP);
  b.ImageShadow = Cmask;
  b.Before = NULL;
  b.After = NULL;
  b.BobVSprite = &vsprite;
  b.BobComp = NULL;  
  b.DBuffer = NULL;
  b.BUserExt =0;
